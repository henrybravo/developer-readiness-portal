...
param(
    ...
    [switch]$NoColor,
    [switch]$Help,
    [string]$Repo = "henrybravo/spec2cloud-agentskills"  # default fork; override with upstream (e.g., EmeaAppGbb/spec2cloud)
)
...
EXAMPLES:
  ...
  .\install.ps1 -Full -Force
    ...
    # Use upstream repo instead of fork
     .\install.ps1 -Full -Repo EmeaAppGbb/spec2cloud
...
function Install-Skills {
    Log-Info "Installing GitHub Copilot skills..."

    $skillsSource = Join-Path $PackageRoot ".github\skills"
    $skillsTarget = Join-Path $TargetDir ".github\skills"

    if (Test-Path $skillsSource) {
        # Copy each skill directory recursively, preserving directory names
        Get-ChildItem -Path $skillsSource -Directory | ForEach-Object {
            $destPath = Join-Path $skillsTarget $_.Name
...
function Write-NextSteps {
    ...
    Write-Host ""
    Write-Host "   ${BLUE}GitHub Copilot Skills (auto-loaded):${NC}"
    Write-Host "     Skills are automatically loaded based on your task context."
    Write-Host "     See .github\skills\ for available skills including:"
    Write-Host "     • spec-validation, shell-baseline-integration, documentation-synthesis"
    Write-Host ""
    Write-Host "3. ${BOLD}Learn more:${NC} https://github.com/$Repo"
    Write-Host ""
