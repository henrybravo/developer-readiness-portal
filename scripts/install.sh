...
OPTIONS:
  ...
  --repo OWNER/NAME   Set repository for docs/links (default: henrybravo/spec2cloud-agentskills)
...
install_skills() {
  log_info "Installing GitHub Copilot skills..."

  # Copy skills directories
  if [ -d "$PACKAGE_ROOT/.github/skills" ]; then
    # Copy each skill directory recursively (remove trailing slash to preserve directory name)
    for skill_dir in "$PACKAGE_ROOT/.github/skills/"*/; do
      if [ -d "$skill_dir" ]; then
        skill_name=$(basename "$skill_dir")
        # Use the skill name explicitly to ensure directory is created, not contents merged
        cp -r "${skill_dir%/}" "$TARGET_DIR/.github/skills/$skill_name" 2>/dev/null || true
      fi
    done
...
print_next_steps() {
  ...
  echo
  echo -e "   ${BLUE}GitHub Copilot Skills (auto-loaded):${NC}"
  echo "     Skills are automatically loaded based on your task context."
  echo "     See .github/skills/ for available skills including:"
  echo "     • spec-validation, shell-baseline-integration, documentation-synthesis"
  echo
  echo -e "3. ${BOLD}Learn more:${NC} https://github.com/${REPO}"
  echo
